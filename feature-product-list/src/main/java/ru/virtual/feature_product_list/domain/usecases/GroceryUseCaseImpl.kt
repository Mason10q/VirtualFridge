...
class GroceryUseCaseImpl @Inject constructor(
    ...
    override fun removeGrocery(listId: Int, productId: Int): Completable =
        repository.removeGroceryFromList(listId, productId)
    ...
    override fun addGrocery(listId: Int, productId: Int): Completable =
        repository.addGroceryToList(listId, productId)
            .observeOn(AndroidSchedulers.mainThread())
    ...
    override fun setMarkState(listId: Int, productId: Int, state: Boolean): Completable =
        Completable.concatArray(
            repository.setMarkState(listId, productId, state)
                .observeOn(AndroidSchedulers.mainThread()),
            if (state) fridgeRepository.addProductToFridge(productId)
            else fridgeRepository.removeProductFromFridge(productId)
        )
